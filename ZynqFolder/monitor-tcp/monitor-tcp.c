...
uint32_t magic_bytes_read_ddr = 0xABCD1221;
typedef struct binary_packet_read_ddr_t {
	uint32_t magic_bytes;	// 0xABCD1221
...
void closeMemoryMap()
{
	...
	if (map_base_xadc != (void*)(-1)) {
		if(munmap(map_base_xadc, MAP_SIZE_XADC) == -1) FATAL;
		map_base_xadc = (void*)(-1);
	}
	// close DMA memory map
	if (map_base_dma != (void*)(-1)) {
		if(munmap(map_base_dma, MAP_SIZE_DMA) == -1) FATAL;
		map_base_dma = (void*)(-1);
	}
		        		...
		        		if (msg_end >= iRequiredBytes)
	        				{
				        		...
				        		// reset our message parsing state variables
				        		bytes_consumed = sizeof(binary_packet_read_ddr_t);
				        		bHaveMagicBytes = false;
				        		iRequiredBytes = sizeof(message_magic_bytes);
